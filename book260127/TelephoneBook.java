...
public class TelephoneBook {
	...
	private Map<String,String> studentTel=new HashMap<>();//여기에서 HashMap객체 미리 생성
	
	//생성자//기본 생성자의 경우 컴파일시 자동으로 만들어지므로 작성 불필요
	...
	//getter
	public Map<String,String> getStudentTel() {
		return this.studentTel;
	}
	//그외 메서드
	//전화번호 등록.수정메서드
	//매개변수:스캐너,클래스의 리스트필드,학생의 이름과 점수를 따로 빼서 저장한 map,전화번호부의 맵필드
	//throws처리하려 했으나 필요없어서 지움....ㅠㅠㅠ
	public boolean telephoneAddList(Scanner sc,List<Student> classStudent,Map<String,String> strMap) throws Exception{
		//2 f왜 다 뛰어넘어서 삭제메서드로 가는가;;
		//2차)
		System.out.println("전화번호를 등록합니다.");
		System.out.println("이름을 입력하세요.(입력종료:-1)");
		System.out.print("이름:");
		...
		for(int i=0;i<classStudent.size();i++) {
			String outName=classStudent.get(i).getName();
			if(outName.equals(name)) {
				findName=outName;
				break;
			}
			else if(i==classStudent.size()-1 && !outName.equals(name)) {
				findName="존재하지 않음";
			}
		}
		out:if(findName.equals(name)) {
			//반 클래스에 학생이 존재할 경우
...
//		}	
	//전화번호부에서 이름삭제 메서드
	public boolean delName(Scanner sc,TelephoneBook telbook) {
		//삭제할 특정 이름
		//삭제 전 이름 존재 여부 확인
		//while문을 멈출 수 있는 플래그?
		boolean stop=false;
		System.out.println("삭제할 이름을 입력하세요.(입력종료:-1)");
		String name=sc.nextLine();
		if(name.equals("-1")) {
		...
		//>>>>>>>>>>>>>>>>
		//맵안에 맵이 들어있는지 묻는꼴이므로 고치기...
		//메인에서 생성한 TelephonBook객체의 Map타입 필드 호출
		Map<String,String> instanceField=telbook.getStudentTel();
		Set<Map.Entry<String, String>> setMap=instanceField.entrySet();
		Iterator<Map.Entry<String, String>> entryIter=setMap.iterator();
		boolean contain=false;
		while(entryIter.hasNext()) {
			Map.Entry<String, String> oneEntry=entryIter.next();
			if(oneEntry.getKey().equals(name)) {
				contain=true;
				break;
			}
		}
		if(contain==true) {
			...
			instanceField.remove(name);
		}else {
			System.out.println("전화번호부에 해당 이름이 저장되어있지 않습니다.");
		}
		return stop;
	}
	public void allPrintBook(Map<String,String> map) {
		//전체 전화번호부 출력
