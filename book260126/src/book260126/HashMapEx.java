...
//		}
		while(true) {
			...
			if(oneStudent==null) {
				...
				//반 클래스의 List필드에 학생의 객체를 저장
				//>>>>>>>>
				//사용할 List를 OneClass클래스의 필드인 classStudent에 저장
		...
		Map<String,String> oneStudentTel=new HashMap<>();
		//매개변수:스캐너,클래스의 리스트필드,학생의 이름과 점수를 따로 빼서 저장한 map,전화번호부의 맵필드
		try {
			oneTelbook.telephoneAddList(sc,studentList,oneStudentTel);
		}catch(Exception e) {
			System.out.println("잘못된 응답을 입력하였습니다.");
		}
		
		//2-3특정이름 삭제
		oneTelbook.delName(sc);
		//2-4전체 전화번호부 출력
		oneTelbook.allPrintBook();
		//스캐너 닫기
		sc.close();
